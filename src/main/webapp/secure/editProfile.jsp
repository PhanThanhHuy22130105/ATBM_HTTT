									...
									<div class="account-settings">
										<div class="user-profile row justify-content-center">
											<div class="user-avatar">
												<img src="${pageContext.request.contextPath}/${sessionScope.user.getImg()}"
						...
						<div class="col-xl-9 col-lg-9 col-md-12 col-sm-12 col-12">
							...
							<div class="card mt-4 border-success">
								...
								<div class="card-body">
									...
									<%-- SỬA LẠI THÀNH SCRIPTLET JAVA CHO THÔNG BÁO XÁC THỰC --%>
									<%
										String successMsg = request.getParameter("msg");
										if (successMsg != null && !successMsg.isEmpty()) {
									%>
									<div class="alert alert-success"><strong>Thành công: </strong><%= successMsg.replace("+", " ") %></div>
									<% } %>
									...
									%>
									<div class="alert alert-danger"><strong>Thất bại: </strong><%= errorMsg %></div>
									<% } %>
							...
							<div class="card mt-4 mb-5 border-danger">
								<div class="card-header bg-danger text-white">
									<h5 class="mb-0"><i class="fa fa-exclamation-triangle"></i> Quản Lý Rủi Ro Khóa</h5>
								</div>
								<div class="card-body">
									<p class="text-danger">Nếu bạn vô tình để lộ file <strong>Private Key</strong> cho người khác, hãy báo mất khóa ngay lập tức. Hệ thống sẽ vô hiệu hóa khóa hiện tại để kẻ gian không thể ký giả mạo tên bạn.</p>

									<%-- SỬA LẠI THÀNH SCRIPTLET JAVA CHO THÔNG BÁO HỦY KHÓA --%>
									<% if (request.getParameter("revokeSuccess") != null) { %>
									<div class="alert alert-warning"><strong>Đã Hủy: </strong>Khóa của bạn đã được đánh dấu là mất. Vui lòng đặt một đơn hàng mới để hệ thống cấp lại khóa mới!</div>
									<% } %>

									<form action="${pageContext.request.contextPath}/revoke-key" method="post" onsubmit="return confirm('Bạn có chắc chắn muốn báo mất khóa không? Mọi giao dịch mới bằng khóa này sẽ bị từ chối.');">
										<input type="hidden" name="userId" value="${sessionScope.user.id}">
										<button class="btn btn-outline-danger font-weight-bold" type="submit">🚨 Báo Mất Private Key</button>
									</form>
								</div>
							</div>
